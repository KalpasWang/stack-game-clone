export default class EventEmiter {
  ...
  remove(_target: { _names: string; _callback: Function; _unsubscribe?: Function }) {
    ...
    names.forEach((_name) => {
      ...
      // remove the callback
      this.callbacks[namespace][value] = this.callbacks[namespace][value].filter(
  ...
  static resolveNames(_names: string) {
    const names = _names;
    const names_step_1 = names.replace(/[^a-zA-Z0-9 ,/.]/g, '');
    const names_step_2 = names_step_1.replace(/[,/]+/g, ' ');
    const names_step_3 = names_step_2.split(' ');
